...
modelo_forward <- step(modelo_vacio, 
                       scope = ~ Age + Gender + Weight + Height + Avg_BPM + Resting_BPM + 
                                Session_Duration + Workout_Type + BMI, 
                       direction = "forward")
...
modelo_inicial <- lm(Calories_Burned ~ Age + Gender + Weight + Height + Avg_BPM + Resting_BPM + Session_Duration + Workout_Type + BMI, data = train)
...
library(MASS)  # Para algunos modelos y funciones
library(caret) # Para manejar particiones de datos, si es necesario

# Suponiendo que tienes un conjunto de datos llamado 'train'
# Asegúrate de ajustar el conjunto de datos de acuerdo a tus necesidades
...
# Si quieres el modelo solo con las variables seleccionadas, puedes usar:
modelo_stepwise$call
