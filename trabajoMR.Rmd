...
```{r}
summary(train$Session_Duration..hours.)
```
...
```{r}
problema <- train |> select(Session_Duration..hours., Calories_Burned)
head(problema, 5)
```
...
model <- lm(Calories_Burned ~ Session_Duration..hours., data = train)
summary(model)
```
...
sesion <- train$Session_Duration..hours.
...
```{r}
correlacion_pearson <- cor(train$Session_Duration..hours., train$Calories_Burned, use = "complete.obs")
...
modelo <- lm(Calories_Burned ~ Age + Gender  + Height..m.+ Weight..kg. + Max_BPM + Avg_BPM + Resting_BPM + Session_Duration..hours.+ Calories_Burned + Workout_Type + Fat_Percentage + Water_Intake..liters. + Workout_Frequency..days.week. + Experience_Level + BMI , data = train)
...
modelo <- lm(Calories_Burned ~ Age + Gender + Weight..kg. + Height..m. + Max_BPM + 
             Avg_BPM + Resting_BPM + Session_Duration..hours. + BMI + 
             Workout_Type + Fat_Percentage + Water_Intake..liters. + 
             Workout_Frequency..days.week. + Experience_Level, data = datos)
