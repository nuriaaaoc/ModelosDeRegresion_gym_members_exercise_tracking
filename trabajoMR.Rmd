...
Este valor así no nos dice nada, solo nos lo va a decir una vez que comparemos el modelo con otros para ver cual nos predice mejor y poder valorar el que mejor nos selecciones. Ahora, vamos a ver los residuos de nuestro modelo.

```{r}
gam.check(modelo_gam)
```

Analizando los resíduos, podemo observar varias cosas, primero de todo que el modelo converge correctamente y es numéricamente estable, además de esto, la elección de k parace estable en todas las variables y podemos ver como las funciones suavizadas Avg_BPM y Sessión duration se comportan practicamente como relaciones lineale, por lo que ahora voy a ver como se ve el modelo depues de reemplazar esas variables suavizadas por las mismas sin suavizar.
