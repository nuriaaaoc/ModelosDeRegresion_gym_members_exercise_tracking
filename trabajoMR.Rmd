...
# Otros modelos de regresión: Modelos Aditivos Generalizados (GAMs).

## Modelo por relaciones.

Dado que nuesro objetivo es modelar nuestra variable objetivo Calories_Burned en función de otras variable. Ahora vamos a hacer el modelo y vamos a ver que posibles cosas podemos ver. Primero de todo, vamos a ver el modelo.
...
En este análisis del modelo, hemos utilizado como predictores Age, Gender, Avg_BPM y Session_Duration. Como podemos ver, este modelo utiliza la familia gaussiana como función de enlace identidad, ya que la función objetivo es continua y aproximadamente simétrica, como hemos visto antes. Este modelo nos indica que ser hombre se asocia a un incremento promedio de 86.98 calorías en comparació con la mujer, además de que el efecto es bastante significativo, evidenciando su relevandia para explicar la variable objetivo.

Por otro lado, las variables suavizadas, tenemos que la variable suavizada de Age, tiene un valor edf mayor a 1, lo que nos indica que puede sigerir patrones curvos o no lineales importantes. Sin embargo, las demás variables, tiene un edf practicamente 1, implicando un comportamiento practicamente lineal con la variable objetivo. Sin embargo, todas las variables suavizadas, son altamanete significativas, indicando que su efectos contribuyen significativamente al modelo.

A parte de las variables, hay otros valores bastante importantes y destacados que hay que resaltar, como el valor de 98.5% de la deviance explicada, lo que nos da a entender que este modelo es capaz de explicar casi toda la variabilidad observada. Por otro lado tenemos el valor -REML de 3376.4, lo que nos ayuda a poder comparar distintos modelo GAM.

Después de ver nuestro modelo, podemos ver que tenemos distintos valores de edf, el de age, como es mayor a 1, nos indica que una posible relación no lineal, sin embargo en los otros, podemos ver como el valor es practicamente 1 sugiriendonos una relacion linal con estas variables. Por otro lado, si vemos el p-valor de las variables suavizadas, nos damos cuenta de que es bastante pequeño, lo que no siguiere que la relación de cada variable con la variable dependiente es estadísticamente significativa. A continuación, vamos a graficar el modelo para ver los efectos suavizados.

```{r}
plot(modelo_gam, main = "Ajuste GAM con Splines Cúbicos", shade = TRUE)
```

En los gráficos anteriores, podemos observar la función suavizda de las variables Age, Avg_BPM y Session_Duration obtenido del ajuste GAM usando splines cúbicos. Viendo los distintos gráficos para cada variable, podemos resaltar más lo que hemos dicho anteriormente con las variable, tenemos que las variable Avg_BPM y Sessión_Duration, se aproximan a una relación lineal, mientras que Age, presenta distintas curvas en su representación. Ahora, vamos a personalizar el gráfico para mejorar la representación. Ahora vamos a ver como se ven con los residuos.
...
Este valor así no nos dice nada, solo nos lo va a decir una vez que comparemos el modelo con otros para ver cual nos predice mejor y poder valorar el que mejor nos selecciones. Ahora, vamos a ver los residuos de nuestro modelo. Por otro lado, como hemos visto antes, las variables de Avg_BPM y Session_Duration, tienen un valor edf muy cercano a 1, por lo que como se pueden considerar lineales, vamos a hacer otro modelo con esos valores sin suavizar para mejorar la interpretabilidad del modelo.
...
Comparando con el modelo anterior, vemos varias cosas, la primera de todoas es que el R-sq (adj) y la Deviance explained no ha cambiado, por lo que si consideramos linealidad entre esas variables no pasa nada. Por otro lado, el valor de -REML, a aumentado un poco, pasando de tener un valor de 3376.4 a 3378, esta diferencia no es muy grande indicando que ambos modelos son bastante válidos estadísticamente. Por lo que de los dos, vamos a seleccionar este, por su simplicidad y explicabilidad. Ahora vamos a ver como se ve gráficamente.

```{r}
plot(modelo_gam_lineal, main = "Ajuste GAM con Splines Cúbicos", shade = TRUE)
```

Después de hacer el modelo con solamente la variable Age suavizada, obtenemos el gráfico enterior, podemos observar que la relación entre la edad y las calorias quemadas no es lineal. A medida que la edad aumenta desde los 20 hasta los 40 años, la función suavizada presenta pequeñas ondulaciones, indicando variaciones en el efecto de la edad, pero después de los 40 años se aprecia un descenso muy marcado en la respuesta, estabilizándose posteriormente. Esto sugiere que alrededor de los 40 años hay un cambio importante en cómo la edad influye en las calorías quemadas. Además, las líneas de puntos representan los intervalos de confianza, mostrando que la estimación es bastante precisa en la mayor parte del rango de edades. Ahora vamos a observar el mismo gráfico pero con los residuos de la variable.

```{r}
plot(modelo_gam_lineal, residuals = TRUE,  pch = 19, col = "black", seWithMean = TRUE, rug = TRUE, main = "Efecto Suavizado de x sobre y")
```

Como podemos ver los residuos se ajustan bastante bien a la curva, además de esto podemos ver que hay una concentración en torna al 40 que pasados los 40 años descience a -50. Ajustando mejor el modelo, vamos a ver si el valor de AIC es mejor que el anterior
...
Comparando este valor con el anterior, podemos observar como ha mejorado, no mucho, pero algo mejor es. Despues de esto, vamos a analizar los residuos del modelo y ver si se podría ajustar bien o no.
...
Primero tenemos el gráfico QQ-plot, el cual nos está comparando los residuos deviance con los varlores teóricos de una distribución normal. La mayoría de los puntos siguen la línea recta, lo que indica que los residuos se distribuyen aproximadamente de forma normal, aunque hay una pequeña desviación en los extremos, típica en muchos modelos.

El segundo gráfico es el de residuos frente a las predicciones lineales, el cuan nos muestra que los residuos se distribuyen de forma relativamente aleatoria alrededor de cero, aunque parece haber una leve forma de “curva”, una ligera heterocedasticidad, lo cual podría deberse a alguna relación no perfectamente modelada, pero no es preocupante en exceso dado el patrón general.

El tercer gráfico es el histograma de los residuos, el cual podemos ver que presenta una forma aproximadamente simétrica, centrada en cero, reforzando la idea principal de que los residuos son razonablemente normales.

Por último, el cuarto gráfico es la respuesta frente a valores ajustados, en el cual se puede apreciar una relación bastante lineal y ajustado, sugiriendo que el modelo percibe bien los valores de la calorias que se queman.

El proceso de ajuste del modelo utilizando el método REML y el optimizador "outer newton" ha sido exitoso, alcanzando la convergencia en tan solo cinco iteraciones, lo cual indica una optimización eficiente y sin dificultades. El rango de gradientes cercano a cero sugiere que se ha llegado a un óptimo real, mientras que la positividad del Hessiano y el rango razonable de los valores propios confirman la estabilidad numérica del ajuste. Además, el hecho de que el rango del modelo sea completo, sin pérdida de parámetros, respalda la correcta especificación del modelo. En cuanto al control del número de bases para el suavizado de la variable Age, los resultados muestran que el número de bases seleccionado ha sido adecuado, dado que el índice k es cercano a 1 y el p-valor es alto, no existiendo evidencia que sugiera la necesidad de incrementar la complejidad del suavizado. En conjunto, esta información respalda que el ajuste del modelo ha sido técnicamente correcto, estable y adecuado para representar las relaciones en los datos.

Viendo que el modelo que solo suaviza la variable Age nos da más ventajas, ahora vamos a ver que modelo no da mejores resultados, el primero es el que ya hemos hecho con REML, pero el segundo utiliza Cross-Validation para encontrar el número óptimo de k para que el modelo se ajuste mejor a los dato.
...
Además, el valor de R-squared ajustado y la desviación explicada son idénticos (98.5%) en ambos modelos, lo cual indica que el ajuste global es excelente en ambos casos. La ligera diferencia en el valor de edf sugiere que el modelo ajustado mediante GCV ha permitido un poco más de flexibilidad en la relación no lineal de la edad, aunque no parece haber un sobreajuste evidente.
...
Con esto podemos ver como se comporta la variable Age en los distinto modelos, la primera columna con el metodo REML y la segunda con GCV.Cp, como podemos apreciar no hay demasiada diferencia entre ambos gráficos, la parte del ajuste se ve que cuando llega a 40 hay una bajada drástica, y vemos que la varianza en cada tramo es prácticamente la misma. Por otro lado, tenemos los residuos que al igual que con los gráficos de arriba, pasa lo msimo, no se aprecian bastantes diferencias entre ambos. Por lo que podemos concluir que en ambos modelos la diferencia en edf no aporta alguna mejorar en la capacidad predctiva.

Viendo esto, hemos dedicidido quedarnos con el modelo hecho por GCV.Cp ya que el valor edf de la variable Age es ligeramente superior permitiendo un poco más de flexibilidad el la relación no lineal que hay entre las variables Calories_Burned y Age, además de esto no parece haber sobreajuste evidente en la variable Age.

## Modelo por Stepwise.

En el modelo de antes lo que hemos hecho ha sido selecciónar las variables según su relación con Calories_Burned y su incorrelación entre ellas. A continuación lo que vamos a hacer es hacer otro moelo para ver si se predice mejor o peor las calorias que se queman utilizando el método de selección de variables de Stepwise. Por lo que primero hacemos el modelo Stepwise y luego seleccionamos esas variables para nuestro modelo.

```{r}
# Modelo completo: Usamos todas las variables
modelo_completo <- lm(Calories_Burned ~ . - Calories_Burned_Binary, data = train)
...
Como podemos ver en el modelo, las variables que han sido seleccionadas son Age, Gender, Weight, Height, Avg_BPM, Resting_BPM, Session_Duration y BMI. Ahora vamos a hacer el modelo GAM con esas variables y vamos a analizarlo como hemos hecho antes.
...
El modelo desarrollado para predecir las calorías quemadas (Calories_Burned) combina efectos paramétricos y suavizados, logrando un excelente ajuste (R² ajustado = 0.985, 98.6% de desviación explicada). La variable Gender muestra un efecto paramétrico positivo y altamente significativo: los hombres tienden a quemar más calorías que las mujeres, controlando por el resto de factores.

Entre los efectos suavizados, destaca s(Age) por su relación compleja y no lineal con las calorías quemadas. Variables como s(Avg_BPM), s(Session_Duration), s(Height), s(Resting_BPM) y s(BMI) presentan relaciones casi lineales, mientras que s(Weight) muestra una moderada no linealidad.
...
Despues de observar las relaciones casi lineales de algunas de las variables suavizadas, lo que vamos a hacer es representar todas las variables suavizadas para observar si la relación que se puede observar llega a ser lineal o  no.

```{r}
plot(modelo_gam_step, main = "Ajuste GAM con Splines Cúbicos", shade = TRUE)
```

Como se puede apreciar en los gráficos, tenemos que la variable Age si que presenta una no lienalidad con la variable objetivo, al igual que la variable Weight. Por otro lado, las variables Height, Avg_BPM, Resting_BPM, Session_Duration y BMI, presentan un ajuste bastante lineal, lo que nos puede dar a entender que suavizar esas funciones solo nos harian tener un modelo más complejo de explicar. Esto se tiene que confirmar con la visisón de sus residuos.

```{r}
plot(modelo_gam_step, residuals = TRUE,  pch = 19, col = "black", seWithMean = TRUE, rug = TRUE, main = "Efecto Suavizado de x sobre y")
```

Efectivamente, al ver los residuos de las variables suavizadas, podemos ver como encontramos residuos no linelaes en las variables Age y Weight, mientras que en el resto de las varibles podemos ver que se ajustan bastante bien a la recta, resaltando su linealidad. A continuación vamos a analizar varias métricas como el AIC y los residuos de nuestro modelo. Primero de todo vamso a ver el valor de AIC.
...
Podemos ver que este valor de primeras es incluso mejor que el valor AIC que hemos obtenido hacendo el modelo GAM por relaciones de variables, lo que nos puede indicar que este modelo tiene un mejor ajuste y que se podría decir que es mejor que el anterior. Por lo que, ahora vamos a suavizar aquellas variable que presenten no lienalidad, Age y Weight, y vamos a estudiar si el modelo es mejor o no.
...
```
El modelo construido para predecir las calorías quemadas (Calories_Burned) combina tanto efectos paramétricos como suavizados, logrando un ajuste sobresaliente (R² ajustado = 0.985, 98.6% de desviación explicada). La variable Gender mantiene un efecto paramétrico positivo y altamente significativo, indicando que los hombres tienden a quemar más calorías que las mujeres, bajo igualdad de condiciones en las demás variables.

En cuanto a los efectos suavizados, s(Age) resalta como una relación compleja y no lineal con la respuesta, mientras que s(Weight) presenta una no linealidad moderada. El resto de variables (Height, Avg_BPM, Resting_BPM, Session_Duration y BMI) se modelan de forma paramétrica, mostrando relaciones principalmente lineales y altamente significativas.

El modelo presenta una estructura más sencilla que favorece la interpretabilidad, al tiempo que mantiene una elevada capacidad predictiva, permitiendo explicar de manera eficiente el comportamiento de las calorías quemadas durante la actividad física.

```{r}
plot(modelo_gam_step_lin, main = "Ajuste GAM con Splines Cúbicos", shade = TRUE)
```

Despues de reducir el número de variables suavizadas, podemos ver mejor como la relación que hay entre las varibles Age y Weight con Calories_Burned es no lienes. Por interpretabilidad y ya que lo que buscamos es hacer un modelo que nos prediga lo mejor posible, escogemos este modelo, que es más facil de interpretar y no tiende al sobreajuste. Para ello, vamos a ver como se ven los residuos.
...
Podemos ver como que los residuos de las variables siguen más o menos la relación no lienal que ya habiamos visto antes. Si que es cierto que se puede apreciar algunos valores atípicos, sobre todo en weight. A continuación vamos a analizar varias métricas como el AIC y los residuos de nuestro modelo. Primero de todo vamso a ver el valor de AIC.
...
Despues de ver el valor AIC que hemos obtenido ante, podemo observar que este valor ha reucido un poco, dandonos a entender que no suavizar ciertas variables no hace un modelo que es más explicable y que además el modelo tiene un mejor ajuste. Por lo que despues de seleccionar el modelo, vamos a ver lso residuos del modelo.
...
Viendo el QQ-Plot, podemos ver que los residuos siguen razonablemente bien la línea de referencia, indicando que la normalidad de los residuos es aceptable, aunque hay pequeñas desviaciones en las colas que podrían sugerir leves outliers o ligera no normalidad.

En el segundo gráfico, de la comparación entre residuos y predicciones lineales, podemo ver que el patrón en forma de U en los residuos indica problemas de no linealidad: el modelo no captura completamente la relación entre la variable predictora y la respuesta. Hay evidencia de que el modelo actual no es suficientemente flexible.

Al analizar los histogramas con los residuos, esto nos muestra residuos aproximadamente simétricos y centrados en cero, lo que refuerza que los residuos tienen una distribución cercana a la normal, con ligeras imperfecciones que no son preocupantes en exceso.

Por último, tenemos las respuestas segun los valores ajustado, en el cual aunque existe una relación creciente entre la respuesta y los valores ajustados, se observa una ligera curvatura en los extremos, indicando que el modelo subestima o sobreestima para valores altos, lo que también sugiere falta de captura de no linealidad.

A parte, tenemos que el modelo ha convergido correctamente y de forma estable, con un gradiente cercano a cero y un Hessiano positivo definido, lo que confirma que se alcanzó un óptimo. El rango del modelo es completo, indicando ausencia de problemas de colinealidad. Además, el chequeo de la dimensión base muestra que no hay indicios de que el número de bases (k) sea insuficiente, ya que los índices k-index están cerca de 1 y los valores p son altos. En conjunto, estos resultados indican que el modelo está bien ajustado y no requiere modificaciones en su configuración actual.

A continación vamos intentar encontrar un grado se suavizado mejor para tener un mejor contros con el sobreajuste. Para ello, vamos a ajustar dos modelos, el primero va a ser con el método REML y el segundo con el método GCV.Cp. Como el modelo hecho por el método REML ya está creado de antes, lo vamos a volver a ver.
...
En cuanto a los efectos suavizados, en ambos modelos las variables Age y Weight han sido modeladas de forma no lineal mediante funciones suavizadas (s(Age) y s(Weight)). Para Age, el grado de complejidad (expresado mediante el valor de edf) es muy parecido en ambos modelos: 8.477 con el método REML y 8.720 con GCV, lo que indica que la relación no lineal entre la edad y las calorías quemadas es consistente y similar en ambos casos. Sin embargo, en el caso de Weight, el valor de edf es notablemente mayor en el modelo GCV (8.016 frente a 4.952 en REML), lo que sugiere que el segundo modelo ha permitido una mayor flexibilidad para capturar la relación entre el peso y las calorías quemadas.

Además, el valor de R-squared ajustado y la desviación explicada son prácticamente idénticos en ambos modelos (98.5%-98.6%), lo cual indica que el ajuste global es excelente en ambos casos. Sin embargo, el modelo ajustado mediante GCV presenta una escala estimada ligeramente más baja (1050.6 frente a 1061.4) y un valor de GCV aceptable, lo que refuerza que ha conseguido un mejor equilibrio entre ajuste y complejidad sin signos evidentes de sobreajuste.

Dado que el objetivo de este trabajo es predecir las calorías quemadas, el modelo ajustado mediante GCV sería el más adecuado para este propósito, ya que consigue una mejor captura de las relaciones no lineales, especialmente en el caso del peso, y ofrece un ajuste global ligeramente superior.
...
A partir del análisis comparativo de los modelos ajustados mediante REML y GCV, se observa que ambos capturan de manera adecuada las relaciones no lineales entre las variables Age y Weight con las calorías quemadas. En ambos casos, la variable Age muestra una relación estable hasta aproximadamente los 40 años, a partir de los cuales se detecta una disminución importante en las calorías quemadas, patrón que es consistente entre ambos métodos. La variable Weight, por su parte, presenta una tendencia negativa en su efecto, siendo más marcada y flexible en el ajuste mediante GCV.
...
No obstante, ambos métodos proporcionan un ajuste de alta calidad, con residuos bien distribuidos y sin patrones de heterocedasticidad evidentes. La elección entre ambos métodos depende del objetivo final: si se prioriza un modelo más preciso y detallado sobre los datos observados, el ajuste mediante GCV es más recomendable; si se busca un modelo más sencillo, estable y potencialmente más robusto ante datos nuevos, el ajuste mediante REML sería la opción más adecuada.

En conclusión, dado que el objetivo principal es modelar de la forma más precisa posible las calorías quemadas en función de las variables consideradas, el modelo ajustado mediante GCV se considera el más adecuado para este caso.

## Modelo por Elastic Net.

```{r}
# Preparamos los datos para hacer el modelo
X1 <- model.matrix(Calories_Burned ~ . - Calories_Burned_Binary, data = train)[, -1]
y <- train$Calories_Burned
...
Las variables seleccionadas son: Age, Gender, Height, Avg_BPM, Resting_BPM, Sessio_Duration, Workout_Type, Fat_Percentage, Workout_Frequency y BMI
...
Con estos gráficos, podemos resaltar más lo que hemos dicho anteriormente con las variable, tenemos que las variable Avg_BPM y Sessión_Duration, se aproximan a una relación lineal, mientras que Age, presenta distintas curvas en su representación. Ahora, vamos a personalizar el gráfico para mejorar la representación.
...
Podemos ver como que los residuos de las variables siguen más o menos la relación no lienal que ya habiamos visto antes. A continuación vamos a analizar varias métricas como el AIC y los residuos de nuestro modelo. Primero de todo vamso a ver el valor de AIC.
...
Este modelo predice las calorías quemadas (Calories_Burned) mediante una combinación de efectos paramétricos y suavizados, logrando un excelente desempeño (R² ajustado = 0.987, explicando el 98.7% de la variabilidad). La variable Gender muestra un efecto positivo fuerte y significativo: los hombres queman más calorías que las mujeres, manteniendo constantes las demás variables.

Entre los efectos paramétricos, Avg_BPM y Resting_BPM resultan significativos, reforzando el rol clave de la frecuencia cardíaca en el gasto energético. Sin embargo, otras variables como Height, Workout_Type, Workout_Frequency y BMI no presentan un impacto significativo, sugiriendo que su influencia es limitada una vez controladas las principales características fisiológicas y de esfuerzo.

Respecto a los efectos suavizados, s(Age), s(Session_Duration) y s(Fat_Percentage) son altamente significativos. Esto indica relaciones no lineales importantes: la edad, la duración de la sesión y el porcentaje de grasa corporal afectan de manera compleja las calorías quemadas.
...
Despues de suavizar solo las variables que tienen un valor por encima de 1 en edf, tenemos esto. Aqui, podemo ver que, como en los otros dos modelo GAM, la variable age, está presente, sin embargo hay algo que nos ha llamado la atención bastante y es que la variable Session_Duration tiene un edf de 6, y al ver su representación podemos ver ligeras curvas, lo que nos puede dar a entender que es una variable que ha sido sobreajustada. Y por último, tenemos la variable Fat_Percentafe en la que tambien se pueden ver ligeras curvas. A continuación vamos a analizar sus residuos para obtener más información.
...
```
Vemos que al calcular el AIC del modelo sin tantas variables suavizadas, vemos que ha disminuido muy poco, solamente 0.2. Lo que nos da a entender que no hay mucha diferencia entre los dos modelos. Despues de haber visto gráficamente la variable Age suavizada, vamos a ver los residuos del modelo seleccionado.
...
En el QQ-Plot, podemo observar que los residuos no son perfectamente normales. Aunque cerca del centro los residuos se alinean bien, en los extremos (colas) se desvían de la línea ideal. Esto sugiere presencia de valores atípicos o colas pesadas.

Por otro lado, vemos que en la gráfica de los residuos y las predicciones, se observa un patrón en forma de curva en la dispersión de los residuos, lo cual indica que el modelo no capta adecuadamente la relación entre las variables. Hay evidencia de no linealidad que el modelo no está modelando.

Al ver los residuos en el histograma se muestra que los residuos están aproximadamente centrados en cero y son algo simétricos, pero no siguen una campana normal perfecta. Hay más concentración cerca de cero y colas más pesadas, apoyando la conclusión del QQ-Plot.

Por último, tenemos el gráfico de respuesto y valores ajustados en la que existe una buena relación lineal general entre los valores predichos y los valores reales, pero se observa mayor dispersión a valores altos, lo que sugiere que el modelo puede perder precisión conforme crecen los valores predichos.

El modelo fue ajustado utilizando el método REML con el optimizador Outer Newton, logrando una convergencia completa en solo 7 iteraciones, lo cual indica un proceso de ajuste eficiente y estable. Los gradientes finales son extremadamente pequeños y la escala estimada de los residuos es razonable, sugiriendo un ajuste sólido. El hecho de que el Hessiano sea positivo definido y que el rango de los valores propios sea adecuado confirma que el mínimo encontrado es válido y que no existen problemas de inestabilidad numérica ni colinealidad severa. Además, el modelo tiene rango completo (37/37), indicando que no hay parámetros redundantes ni mal definidos.

En cuanto a la revisión de la dimensión base (k-check), las funciones suavizadas para "Age", "Session_Duration" y "Fat_Percentage" no presentan señales de falta de flexibilidad: los valores de k-index están cerca de 1 y los p-valores son suficientemente altos, lo que significa que no es necesario aumentar el número de bases para capturar adecuadamente la forma de las relaciones no lineales.

Con esto podemos ver que el modelo converge de forma correcta, es tstable, bien identificada y la elección de la complejidad de los suavizadores es apropiada. A continación vamos intentar encontrar un grado se suavizado mejor para tener un mejor contros con el sobreajuste. Para ello, vamos a ajustar dos modelos, el primero va a ser con el método REML y el segundo con el método GCV.Cp. Como el modelo hecho por el método REML ya está creado de antes, lo vamos a volver a ver.
...
Dado que el objetivo es predecir de manera precisa y generalizable las calorías quemadas a partir de las variables disponibles, consideramos que el modelo ajustado con REML es el más adecuado. Este proporciona un equilibrio más robusto entre ajuste y capacidad de generalización, siendo menos propenso a sobreajustar los datos. Por tanto, seleccionamos el modelo basado en REML como la mejor opción para la predicción exacta de las calorías quemadas.
...
Como hemos observado, hemos hecho tres modelos GAMs distintos para predecir el mismo objetivo, ya que lo que buscamos es el mejor modelo. Despues de analizar detalladamente cada gráfico y de ver los residuos tanto de las variables suavizada, vamos a seleccionar uno.

El primer modelo, es un modelo que hemos hecho según las relaciones entre las variables, en este modelo se puede ver que el modelo tiene una alta explicabilidad de la deviance del 98.5%, además de que tiene un R-sq ajustado bastante alto, muy cercano a 1. Tenemos un valor -REML  de 3378 y un valor AIC de  6753.735.

El segundo modelo lo hemos hecho median la selección de variables de Stepwise, en este modelo hemos observado que el valor de nuestro AIC a disminuido un poco haciendo el mejor modelo de todos. Además de esto, tenemo que el modelo tiene una explicabilidad de la deviance del 98.6% y el R-sq ajustado es de 0.985, igual que el primer modelo, al igual que tenemos un valor de -REML de 3359.8 siendo el más bajo hasta el momento.

Por último tenemos el modelo que se ha creado mediante la selección de variables con Elastic Net, en este modelo se ha observado que el AIC a aumentado mucho, incluso más que el primer modelo, al igual que hemos podido observar como hay variables, como Session_Duration, que presentan un poco se sobreajuste. Por esto y porque escoge casi todas las variables del dataset, creeemos que es el peor modelo de todos, cierto es que tiene una devianza explicada del 98.7% y un R-sq ajustado de 0.987 lo que nos dice que el modelo es muy preciso, además de esto tiene in valor -rm de 3329.8, pero hemos decidido dar prioridad a evitar sobredispersión con bastante explicabilidad a un modelo que sea super explicable pero que esté sobreajustado.

Por lo que una vez vistos los modelos, hemos optado por seleccionar el segundo modelo, ya que es bastante explicable y sencillo, además de su precisión para el análisis
